...
## Routes
| Method 	| Route 	| Controller 	| Action 	| Return 	| Body Params
|-	|-	|-	|-	|-	|- |
| POST 	| /auth/register 	| UserController 	| Should register a User given an email, user and password 	| Return User info with a jWT 	| email, name, password |
| POST 	| /auth/login 	| UserController 	| Should login a user given an email and password 	| Return User info with a jWT 	| email, password |
